...
@section('title')
    <title>Prontoform At&t - Roles de Usuarios</title>
@endsection

@section('content')
<main id="crud" role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4" crud="roles">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
        <h1 class="h1">Roles de Usuarios</h1>
    </div>
    <form @submit.prevent="createCrud" method="POST" id="createroles" class="input-group mb-3">
        @csrf
        <input type="text" class="form-control" placeholder="Indicar tipo de usuario" name="name">
    ...
    <table id="Area_table" class="table display">
        ...
        <tbody>
            <tr v-for="type in getCrudList">
                <td>@{{type.name}}</td>
                <td>
                    <a href="#" class="btn btn-warning" v-on:click.prevent="permissionsModal(type.id)">Permisos</a>
                    <a href="#" class="btn btn-danger" v-on:click.prevent="deleteCrud(type.id)">Eliminar</a>
                </td>
            </tr>
        </tbody>
    </table>
    ...
    <script>
        new Vue({
            ...
            methods: {
                ...
                createCrud: function(){
                    this.fd = new FormData(document.getElementById('createroles'));
                ...
                editRoles: function(){
                    axios.put('roles/' + this.permissions.id, this.permissions).then(response => {
                        $('#edit').modal('hide');
                        this.getCrud();
                    });
                },
